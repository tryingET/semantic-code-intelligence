...
├── src/mcp/tool-call-executor.ts
│   # MCP server-side validation/timeout execution helper that preserves JSON-RPC error envelopes
...
└── src/mcp/tool-workflow-runner.ts
    # MCP adapter bridge from validated core workflows to MCP tool-result envelopes
```
