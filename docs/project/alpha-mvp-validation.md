...
- `explore_symbol_impact` parity coverage proves that compact remains decision-only, standard adds normalized bounded evidence, and debug adds separately bounded/redacted diagnostics through HTTP, direct MCP, CLI, and the registered native Pi tool;
...
For `explore_symbol_impact`, parity includes the same progressive mode semantics through HTTP tools/call, direct `MCPAdapter`, and fresh CLI workflow processes. A separate fresh Pi process must load the registered native tool and execute it through MCP stdio; source tests and mock extension registration do not substitute for that runtime proof. Standard detail output is capped at 24 KiB, debug detail output at 36 KiB, the complete packet at 48 KiB, evidence output lists at 12 items per section, analyzed backend collections and provenance fields at 4,096 entries per section, analyzed limitation lists at 64 entries, and debug raw fragments at 768 bytes per subcall with bounded keys, arrays, depth, and nodes. Validation must measure confirmed, unconfirmed, degraded, and high-fanout output sizes; exercise huge metadata, limitation, and path inputs; and reject Unix/Windows/UNC absolute paths, connection credentials, common token/key/PEM secrets, environment-shaped metadata/values, or stack-trace leakage. Native Pi validation must also prove malformed or oversized producer output becomes valid fail-closed JSON before parsing or rendering.
