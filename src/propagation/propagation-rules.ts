...
import type { Change, Concept, Suggestion } from '../types/core';
import type { PropagationContext } from './knowledge-spreader';
...
export class GetterSetterSyncRule extends PropagationRule {
    constructor() {
        super('getter_setter_sync', 'Synchronizes getter and setter method names', 8);
    }
...
export class InterfaceImplementationSyncRule extends PropagationRule {
    constructor() {
        super('interface_implementation_sync', 'Synchronizes interface and implementation method names', 9);
    }
...
export class TestFileSyncRule extends PropagationRule {
    constructor() {
        super('test_file_sync', 'Synchronizes test files with source file changes', 6);
    }
    ...
    private isTestVariant(testName: string, sourceName: string): boolean {
        ...
        return testSuffixes.some((suffix) => testName === sourceName + suffix);
    }
}
...
export class ServiceControllerPatternRule extends PropagationRule {
    constructor() {
        super('service_controller_pattern', 'Propagates changes between service and controller layers', 7);
    }
...
export class NamingConventionRule extends PropagationRule {
    constructor() {
        super('naming_convention', 'Maintains naming convention consistency', 4);
    }
    ...
    async apply(change: Change, context: PropagationContext): Promise<Suggestion[]> {
        ...
        if (conventionChange) {
            ...
            const candidates = await this.findConventionCandidates(conventionChange, context);
    ...
    private detectConventionChange(
        oldName: string,
        newName: string
    ): {
        from: string;
    ...
    private applyConvention(identifier: string, convention: { from: string; to: string; type: string }): string | null {
