...
export function definitionToApiResponse(def: Definition) {
    ...
    return {
        uri: normalizeUri(def.uri),
        range,
        kind: (def as any).kind,
        name: (def as any).name,
        confidence: (def as any).confidence,
        source: (def as any).source,
        layer: (def as any).layer,
    };
}
...
export function referenceToApiResponse(ref: Reference) {
    ...
    return {
        uri: normalizeUri(ref.uri),
        range,
        kind: (ref as any).kind,
        name: (ref as any).name,
        confidence: (ref as any).confidence,
        source: (ref as any).source,
        layer: (ref as any).layer,
    };
}
