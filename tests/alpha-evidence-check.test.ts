...
const requiredAlphaTools = ['get_snapshot', 'read_file', 'text_search', 'symbol_search', 'find_definition', 'find_references', 'ast_query', 'graph_expand', 'recommend_checks', 'propose_patch', 'run_checks'];
...
function goodEvidenceFiles() {
  return {
    'alpha-mvp-dogfood.json': { ok: true, schema: 'fixture-alpha', summary: requiredAlphaTools.map((name) => ({ name, success: true, elapsedMs: 1 })) },
    ...
    'structural-workflow-dogfood.json': {
      ...
      calls: [
        ...
        { name: 'structural_patch_checks', success: true, elapsedMs: 1, sample: { payload: { applied: false } } },
      ],
    },
    ...
    'safe-write-dogfood.json': {
      ...
      calls: [
        { payload: { mode: 'preview_validate', applied: false, checkRecommendations: { workflow: 'recommend_checks' }, validationPlan: { schema: 'semantic-code-intelligence.validation_plan.v1' } }, elapsedMs: 1 },
        { payload: { applied: true, verification: { appliedDiffMatchesSnapshot: true }, validationPlan: { verification: { appliedDiffMatchesSnapshot: true } } }, elapsedMs: 1 },
        { payload: { applied: true, ok: false, verification: { appliedDiffMatchesSnapshot: false }, validationPlan: { verification: { appliedDiffMatchesSnapshot: false } } }, elapsedMs: 1 },
      ],
    },
    'validation-plan-comparison.json': { ok: true, schema: 'fixture-validation-plan-comparison', comparedPlanCount: 2, drift: [] },
  };
}
  ...
  test('redacts malicious evidence detail strings in operator-facing JSON', () => {
    const root = makeEvidenceRoot({
      'alpha-mvp-dogfood.json': {
        ...
        summary: requiredAlphaTools.concat('TOKEN_SECRET=tool-secret').map((name) => ({ name, success: true, elapsedMs: 1 })),
      },
