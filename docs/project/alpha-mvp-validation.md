...
- CLI fallback proves `safe_write` as the autonomous-safe write path: preview/check by default, guarded apply refusal without `ALLOW_SNAPSHOT_APPLY=1`, risk classification, rollback command, and brief output;
