...
if (safeWrite) {
    ...
    const cleanApplyVerified = calls.some((call) => call?.scenario === 'clean_apply' && call?.payload?.applied === true && call?.payload?.verification?.appliedDiffMatchesSnapshot === true);
    const cleanApplyValidationPlanVerified = calls.some((call) => call?.scenario === 'clean_apply' && call?.payload?.applied === true && call?.payload?.validationPlan?.verification?.appliedDiffMatchesSnapshot === true);
    const dirtyBaseVerified = safeWrite?.assertions?.dirtyTouchedFileVerificationPreservesBase === true && calls.some((call) => call?.scenario === 'dirty_base_apply' && call?.payload?.applied === true && call?.payload?.ok === true && call?.payload?.verification?.appliedDiffMatchesSnapshot === true);
    const dirtyBaseValidationPlanVerified = safeWrite?.assertions?.validationPlanDirtyTouchedFileVerificationPreservesBase === true && calls.some((call) => call?.scenario === 'dirty_base_apply' && call?.payload?.applied === true && call?.payload?.ok === true && call?.payload?.validationPlan?.verification?.appliedDiffMatchesSnapshot === true);
    ...
    checks.push({ name: 'safe_write_applied_state_verified', ok: cleanApplyVerified });
    checks.push({ name: 'safe_write_validation_plan_applied_state_verified', ok: cleanApplyValidationPlanVerified });
    checks.push({ name: 'safe_write_dirty_base_verified', ok: dirtyBaseVerified });
    checks.push({ name: 'safe_write_validation_plan_dirty_base_verified', ok: dirtyBaseValidationPlanVerified });
    checks.push({
