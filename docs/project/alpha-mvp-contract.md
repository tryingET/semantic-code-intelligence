...
| `explore_symbol_impact` | Preferred impact workflow for bounded definition/reference/graph inspection around a symbol and optional file context. It fails closed unless a locatable definition/declaration is confirmed. | A compact default packet with one confirmed definition, deduplicated semantically ranked impacted files, edit-risk and public-API/state/registry/test signals, next reads, bounded limitations, and explicit truncation; raw AST/backend/provenance evidence only in `standard`/`debug` mode. |
