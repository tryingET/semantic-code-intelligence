...
- `patch_checks_in_snapshot` and `safe_write` return a compact `validationPlan` summary with selected commands, per-command check receipts, recommendation evidence, aggregate check result, snapshot artifacts, apply posture, and rollback links where applicable.
