...
- `tests/evidence-review-claim-model.test.ts` covers read-only rendering, selected-vs-recommended command separation, absence states, first-class graph limitation visibility, limitation reference integrity, markdown-forgery neutralization, oversized-input refusal, workspace input containment, symlink-escape refusal, missing/non-regular input refusal, rollback absence, production-readiness caveats, and non-authority-durable local validation execution.
