...
- **Regression coverage**: direct service tests plus MCP/HTTP/CLI parity tests should protect workflow behavior while keeping the adapter boundary visible. Maintained MCP server entrypoints belong in the package build matrix: `bun run build:all` builds stdio MCP, Streamable HTTP MCP, and enhanced MCP, while `just test-mcp-enhanced` smokes the built `dist/mcp-enhanced/mcp-enhanced.js` artifact. MCP HTTP owns browser CORS policy with `MCP_HTTP_CORS_ORIGIN`; its default browser CORS policy is loopback-origin only, so reverse-proxy/external exposure must configure an explicit origin allowlist. The adapter config keeps stale SSE fields only as ignored compatibility inputs.
