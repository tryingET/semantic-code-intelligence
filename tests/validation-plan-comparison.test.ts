...
function writeEvidence(
        ...
        safeWritePlan(),
        {
            ...
            plan: safeWritePlan({
                ...
                verification: {
                    ...
                    applyGuardSatisfied: true,
                    applied: true,
                    appliedDiffMatchesSnapshot: true,
                },
            }),
        },
        {
    ...
    const plans = (Array.isArray(safePlans) ? safePlans : [safePlans]).map((entry: any) =>
        entry?.plan ? entry : { scenario: undefined, plan: entry }
    );
    mkdirSync(root, { recursive: true });
    writeFileSync(
        join(root, 'recommend-checks-dogfood.json'),
        JSON.stringify(
            {
                schema: 'semantic-code-intelligence.recommend_checks_dogfood.v1',
                ok: true,
                calls: [
                    { caseName: 'patch_checks_recommendations_threaded', payload: { validationPlan: recommendPlan } },
                ],
            },
            null,
            2
    ...
    writeFileSync(
        join(root, 'safe-write-dogfood.json'),
        JSON.stringify(
            {
                schema: 'semantic-code-intelligence.safe_write_dogfood.v1',
                ok: true,
                calls: plans.map((entry) => ({
                    scenario: entry.scenario,
                    payload: { mode: entry.plan.mode || 'preview_validate', validationPlan: entry.plan },
                })),
            },
            null,
            2
...
describe('validation plan comparison graph context', () => {
    test('passes when generated evidence includes stable graph review context', () => {
        ...
        expect(report.interpretation.proves).toContain(
    ...
    test('fails closed when applied safe_write validationPlan omits applied-diff match state', () => {
        ...
        expect(report.ok).toBe(false);
        expect(report.drift.some((item: any) => item.failures.includes('safe_write_verification_incomplete'))).toBe(
            true
        );
    });
    ...
    test('reports schema drift instead of dropping changed validation plans', () => {
        ...
        writeEvidence(
            root,
            validationPlan({ schema: 'semantic-code-intelligence.validation_plan.v2' }),
            safeWritePlan({ schema: 'semantic-code-intelligence.validation_plan.v2' })
        );
    ...
    test('fails closed when safe_write bundle lacks clean and dirty-base verification coverage', () => {
        const root = makeRoot();
        ...
        expect(report.ok).toBe(false);
        expect(
            report.drift.some((item: any) => item.failures.includes('safe_write_verification_coverage_missing'))
        ).toBe(true);
    });
        ...
        writeEvidence(
            ...
            validationPlan({
                ...
                graphImpact: {
                    seed: {
                        kind: 'file',
                        value: `src/example.ts SECRET_KEY=graph-secret ${process.cwd()} /tmp/private-graph`,
                    },
                    requestedEdges: ['imports'],
                    evidence: [
                        {
                            edge: `imports TOKEN_SECRET=edge-secret ${process.cwd()} /tmp/private-edge`,
                            count: 1,
                            status: 'evidence',
                        },
                    ],
    ...
    test('rejects oversized and symlinked generated evidence inputs without leaking contents', () => {
        ...
        writeFileSync(
            join(root, 'recommend-checks-dogfood.json'),
            `{"schema":"semantic-code-intelligence.recommend_checks_dogfood.v1","padding":"${'x'.repeat(10 * 1024 * 1024)}"}`
        );
    ...
    test('refuses to write comparison output through a symlink', () => {
        ...
        const outsideRoot = makeRoot();
        symlinkSync(
            join(outsideRoot, 'validation-plan-comparison.json'),
            join(root, 'validation-plan-comparison.json')
        );
    ...
    test('fails closed when graph edge count is missing', () => {
        ...
        expect(report.ok).toBe(false);
        expect(
            report.drift.some((item: any) =>
                item.actual.graphImpactValidationFailures.includes('graph_edge_count_invalid')
            )
        ).toBe(true);
    });
});
