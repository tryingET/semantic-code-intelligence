...
If checks exceed the interactive band, reduce check scope when truthful (for example narrow test files) or tell the operator the workflow is now validation-heavy rather than navigation-heavy. Alpha evidence history attributes total selected-command receipt time separately from residual SCI/tool overhead when receipts are available; this prevents genuinely command-dominated validation drift from being mistaken for SCI tool latency while preserving total elapsed-time budget failures.
