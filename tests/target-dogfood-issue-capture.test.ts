    ...
    return failedEvidence({
        ...
        calls: [
            ...
            },
            {
                ...
                elapsedMs: 20,
                stderrClean: true,
                observation: 'preview/check succeeded',
                payload: {
...
function runCapture(input: unknown, args: string[] = []) {
    ...
    const result = spawnSync(
        'bun',
        ['run', script, '--input', relative(process.cwd(), inputPath), '--stdout-only', ...args],
        {
            cwd: process.cwd(),
            encoding: 'utf8',
        }
...
describe('target dogfood issue capture', () => {
    test('captures failed target dogfood as a redacted issue candidate', () => {
        const { issue, result } = runCapture(failedEvidence(), [
            '--operator-note',
            `operator saw ${process.cwd()} and SECRET_KEY=abc123`,
        ]);
                ...
                calls: [
                    {
                        ...
                        exitCode: 0,
                        success: true,
                        payload: { workflow: 'graph_expand', ok: false },
                    },
                ],
            })
        );
    ...
    test('writes an output artifact inside the SCI workspace when requested', () => {
        ...
        expect(result.status, result.stderr).toBe(0);
        expect(JSON.parse(readFileSync(outputPath, 'utf8')).schema).toBe(
            'semantic-code-intelligence.target_dogfood_issue.v1'
        );
    });
    ...
    test('rejects unsupported input schema without reflecting untrusted schema text', () => {
        ...
        const result = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), inputPath), '--stdout-only'],
            { cwd: process.cwd(), encoding: 'utf8' }
        );
    ...
    test('rejects oversized evidence input before parsing', () => {
        ...
        writeFileSync(
            inputPath,
            `{"schema":"semantic-code-intelligence.target_validation_plan_dogfood.v1","padding":"${'x'.repeat(10 * 1024 * 1024)}"}`
        );
        const result = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), inputPath), '--stdout-only'],
    ...
    test('rejects input path escapes and symlink escapes without leaking target contents', () => {
        ...
        const symlinkResult = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), linkPath), '--stdout-only'],
            { cwd: process.cwd(), encoding: 'utf8' }
        );
        ...
        const escapeResult = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), outsidePath), '--stdout-only'],
            { cwd: process.cwd(), encoding: 'utf8' }
        );
    ...
    test('rejects output paths through symlinked parents before creating outside directories', () => {
        ...
        const result = spawnSync(
            ...
            [
                ...
                relative(process.cwd(), inputPath),
                '--output',
                relative(process.cwd(), join(linkPath, 'new-child', 'issue.json')),
            ],
            { cwd: process.cwd(), encoding: 'utf8' }
        );
    ...
    test('rejects output path escapes and output symlinks', () => {
        ...
        const escapeResult = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), inputPath), '--output', outsidePath],
            { cwd: process.cwd(), encoding: 'utf8' }
        );
        expect(escapeResult.status).not.toBe(0);
