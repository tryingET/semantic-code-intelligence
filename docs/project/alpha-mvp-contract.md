...
- `explore_symbol_impact` defaults to progressive disclosure: `compact` preserves the decision-only schema and fail-closed unconfirmed response; `standard` emits allowlisted normalized evidence under a 24 KiB detail budget; `debug` adds allowlisted diagnostics and at most one 768-byte sanitized raw fragment per subcall under a 36 KiB detail budget; every complete packet is capped at 48 KiB or replaced by a valid fail-closed result. Every evidence section has a 12-item output cap and 4,096-item analysis cap; provenance scans stop after 4,096 fields and limitation scans after 64 entries. Counts distinguish exact from analysis-truncated observations, and item, shape, raw-fragment, and byte omissions remain explicit. Unix/Windows/UNC absolute or outside-workspace paths, connection credentials, common token/key/PEM secrets, environment-shaped metadata/values, and stack traces are never disclosed. Only workspace-contained exact-name definitions/declarations confirm a symbol; every confirmed definition file contributes to risk, and truncating displayed files never suppresses aggregate risk signals.
