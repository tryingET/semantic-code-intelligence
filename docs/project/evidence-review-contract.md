...
24. SCI-local handoff-readiness indicators use neutral `present`/`missing`/`not_asserted` statuses rather than `satisfied` or readiness wording;
25. handoff gate details distinguish schema indicators, observed statuses, and external authority requirements without a generic overloaded `evidence` field;
26. absence-state readiness cannot pass by vacuous truth when no evidence artifact statuses are observed;
27. docs strict and direction check pass;
28. if runtime contracts change, `bun run alpha:mvp:check` still passes.
