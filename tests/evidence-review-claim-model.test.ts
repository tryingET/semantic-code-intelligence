...
function assertClaimModel(review: any) {
    ...
    expect(
        review.handoffReadiness.gates.find((gate: any) => gate.id === 'claim-model-fields')?.schemaIndicatorsPresent
    ).toContain('claims');
    expect(review.handoffReadiness.gates.find((gate: any) => gate.id === 'absence-states')?.observedStatuses).toContain(
        'observed'
    );
    expect(
        review.handoffReadiness.gates.find((gate: any) => gate.id === 'absence-states')?.schemaIndicatorsPresent
    ).toEqual([]);
    expect(review.handoffReadiness.gates.find((gate: any) => gate.id === 'host-owner-acceptance')?.status).toBe(
        'missing'
    );
    expect(
        review.handoffReadiness.gates.find((gate: any) => gate.id === 'host-owner-acceptance')
            ?.externalAuthorityRequired
    ).toBe(true);
    expect(review.handoffReadiness.gates.find((gate: any) => gate.id === 'sample-and-test-evidence')?.status).toBe(
        'not_asserted'
    );
...
describe('evidence review claim model', () => {
    ...
    test('markdown output renders claim model sections', () => {
        ...
        expect(output).toContain(
            'bun test tests/example.test.ts — ok=true; exitCode=not recorded; timedOut=false; elapsedMs=not recorded'
        );
        ...
        const packet = {
            ...
            previewFirstMutation: {
                validationPlanSample: {
                    ...
                    workflow: 'patch_checks_in_snapshot',
                    commands: {
                        selected: ['echo claimed but not evidenced'],
                        recommendedMinimum: [],
                        recommendedBroader: [],
                        recommendationsAppliedToSelected: false,
                    },
    ...
    test('selected check command failure contradicts aggregate clean-pass claims', () => {
        ...
        expect(review.limitations.find((item: any) => item.id === 'validation-execution-limitation-1')?.severity).toBe(
            'blocking'
        );
    });
    ...
    test('duplicate selected commands consume distinct execution evidence entries', () => {
        const plan = clonePlan({
            commands: {
                selected: ['bun test tests/example.test.ts', 'bun test tests/example.test.ts'],
                recommendedMinimum: [],
                recommendedBroader: [],
                recommendationsAppliedToSelected: false,
            },
    ...
    test('validation execution is not authority durable without explicit AK evidence', () => {
        ...
        expect(artifactById(review, 'validation-execution')?.citationRequirement).toContain(
            'local summary output alone is not authority-durable evidence'
        );
    });
    ...
    test('snapshot artifact durability distinguishes materialized files, snapshot pointers, and path escapes', () => {
        ...
        const { stdout: snapshotStdout } = runSummary(
            clonePlan({ artifacts: { overlayDiff: 'snapshot://example/overlay.diff' } }),
            ['--format', 'json']
        );
        ...
        expect(artifactById(snapshotReview, 'snapshot-artifacts')?.citationRequirement).toContain(
            'snapshot:// references are pointers'
        );

        const { stdout: escapedStdout } = runSummary(
            clonePlan({ artifacts: { overlayDiff: '../outside/overlay.diff' } }),
            ['--format', 'json']
        );
    ...
    test('rollback posture distinguishes inverse patches, preview no-op, and applied mutation without rollback', () => {
        ...
        const { stdout: inverseStdout } = runSummary(
            clonePlan({
                apply: { applied: true },
        ...
        expect(inverseReview.rollback).toMatchObject({
            available: true,
            status: 'inverse_patch_materialized',
            inversePatch: relativeInversePatch,
        });
        expect(artifactById(inverseReview, 'rollback')).toMatchObject({
            observedStatus: 'observed',
            durability: 'materialized_local',
        });
        expect(claimById(inverseReview, 'rollback-posture')?.status).toBe('supported');

        const { stdout: previewStdout } = runSummary(clonePlan({ apply: { applied: false }, rollback: {} }), [
            '--format',
            'json',
        ]);
        const previewReview = JSON.parse(previewStdout);
        expect(previewReview.rollback).toMatchObject({
            available: false,
            notNeeded: true,
        ...
        const { stdout: missingStdout } = runSummary(clonePlan({ apply: { applied: true }, rollback: {} }), [
            '--format',
            'json',
        ]);
        const missingReview = JSON.parse(missingStdout);
        expect(missingReview.rollback).toMatchObject({
            available: false,
            notNeeded: false,
            status: 'unavailable_after_apply',
        });
        ...
        expect(missingReview.limitations.find((item: any) => item.id === 'rollback-limitation-1')?.severity).toBe(
            'blocking'
        );
    });
        ...
        const packet = {
            ...
            graphImpact: {
                fileImpact: {
                    hasImpactEvidence: false,
                    counts: {},
                    limitations: ['file fallback limitation'],
                    planningHints: [],
                },
    ...
    test('markdown output neutralizes forged headings and links in caller-controlled command text', () => {
        ...
        const plan = clonePlan({
            commands: {
                selected: [forgedCommand],
                recommendedMinimum: [forgedCommand],
                recommendedBroader: [],
                recommendationsAppliedToSelected: false,
            },
        ...
        expect(output).toContain(
            'true ⏎ ## FORGED CHECK STATUS ⏎ \\[secret\\]\\(file:///tmp/secret\\) — ok=true; exitCode=not recorded; timedOut=false; elapsedMs=not recorded'
        );
    ...
    test('markdown output neutralizes forged headings and inline markdown in untrusted limitation text', () => {
        ...
        expect(markdown).toContain(
            '&lt;/li&gt; ⏎  ⏎ ## FORGED GREEN STATUS ⏎ - \\*\\*Production-ready: true\\*\\* ⏎ - \\[Applied\\]\\(file:///tmp/secret\\)'
        );
    ...
    test('summary rejects oversized evidence inputs before parsing', () => {
        ...
        writeFileSync(
            inputPath,
            `{"schema":"semantic-code-intelligence.validation_plan.v1","padding":"${'x'.repeat(10 * 1024 * 1024)}"}`
        );
    ...
    test('summary rejects evidence inputs outside the workspace without leaking contents', () => {
        ...
        writeFileSync(
            inputPath,
            JSON.stringify({ ...sampleValidationPlan(), graphImpact: { limitations: ['outside-secret-marker'] } })
        );
    ...
    test('summary rejects workspace symlink escapes without leaking target contents', () => {
        ...
        writeFileSync(
            outsidePath,
            JSON.stringify({ ...sampleValidationPlan(), graphImpact: { limitations: ['symlink-secret-marker'] } })
        );
        ...
        const result = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), linkPath), '--format', 'markdown'],
            {
                cwd: process.cwd(),
                encoding: 'utf8',
            }
    ...
    test('read boundary rejects evidence input replaced after stat before open', async () => {
        ...
        writeFileSync(
            replacementPath,
            JSON.stringify({ ...sampleValidationPlan(), graphImpact: { limitations: ['replacement-secret-marker'] } })
        );

        expect(() =>
            readJson(relative(process.cwd(), inputPath), {
        ...
        expect(() =>
            readJson(relative(process.cwd(), inputPath), {
                afterOpenStat: () =>
                    writeFileSync(
                        inputPath,
                        JSON.stringify({ ...sampleValidationPlan(), padding: 'mutated-after-open' })
                    ),
            })
    ...
    test('summary rejects missing, unreadable, and non-regular evidence inputs before parsing', () => {
        ...
        const missingResult = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), missing), '--format', 'json'],
            {
                cwd: process.cwd(),
                encoding: 'utf8',
            }
        ...
        if (typeof process.getuid !== 'function' || process.getuid() !== 0) {
            ...
            const unreadableResult = spawnSync(
                'bun',
                ['run', script, '--input', relative(process.cwd(), unreadable), '--format', 'json'],
                {
                    cwd: process.cwd(),
                    encoding: 'utf8',
                }
        ...
        const dirResult = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), inputDir), '--format', 'json'],
            {
                cwd: process.cwd(),
                encoding: 'utf8',
            }
    ...
    test('summary rejects FIFO evidence inputs without blocking when mkfifo is available', () => {
        ...
        const result = spawnSync(
            'bun',
            ['run', script, '--input', relative(process.cwd(), fifoPath), '--format', 'json'],
            {
                cwd: process.cwd(),
                encoding: 'utf8',
                timeout: 2000,
    ...
    test('summary rejects unsupported formats before parsing input and without reflecting untrusted format text', () => {
        ...
        writeFileSync(
            inputPath,
            '{"schema":"semantic-code-intelligence.validation_plan.v1","secret":"format-before-parse-marker", BAD'
        );
    ...
    test('summary rejects unsupported extract modes before parsing input and without reflecting untrusted extract text', () => {
        ...
        writeFileSync(
            inputPath,
            '{"schema":"semantic-code-intelligence.alpha_evidence_packet.v1","secret":"extract-before-parse-marker", BAD'
        );
    ...
    test('summary reports malformed JSON without source paths, code frames, or input content', () => {
        ...
        writeFileSync(
            inputPath,
            '{"schema":"semantic-code-intelligence.validation_plan.v1","secret":"json-secret-marker", BAD'
        );
